...
/* nestedFunction was able to access the variable internal because it was nested inside of myFunction, making nestedFunction a closure */
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const newArray = arr.map(item => item.animal_name.toLowerCase());
    return newArray;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const newArray = arr.filter(item => item.population < 5);
    return newArray;
  }
...
CuboidMaker.prototype.surfaceArea = function(){
  const total =  2 * (this.length * this.width + this.length * this.height + this.width * this.height);
  return total;
};
